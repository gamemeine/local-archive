...
def test_get_or_create_user_returns_existing():
    # Arrange
    session = setup_memory_db()
    existing = User(id='u2', email='e2@mail', display_name='Existing', role='r', auth_provider='p')
    session.add(existing)
    session.commit()
    # Act
    user = get_or_create_user(db=session, id='u2', email='x', display_name='x', role='x', auth_provider='x')
    # Assert
